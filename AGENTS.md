...
**Backend kinds:** `src/backend/mod.rs` defines `enum Backend { Api(Box<AgentLoop>), ClaudeCli(ClaudeCliDriver), Stub(Box<StubBackend>) }`. `main.rs` builds one variant at startup from the resolved config entry and never switches at runtime. The build path itself lives in `src/backend/factory.rs`'s `BackendFactory`, extracted from `main.rs`. It runs again at runtime, on a possibly different entry, whenever the `Task` tool dispatches a subagent. See "Task tool (subagent dispatch)" below. The `Stub` variant, in `src/backend/stub.rs`, answers each turn from a scripted `StubTurn` list, with no network call and no child process. It exists only so subagent tests can exercise dispatch, the registry, and the turn caps without hitting a real API or spawning `claude`. `BackendFactory::with_stub` is the one way to reach it, and that method is `#[cfg(test)]`. The `backends` map in settings.json can only ever produce an `Api` or `ClaudeCli` entry, so a stub cannot appear in a normal run by accident.
...
**Voice reply mode:** while text to speech is on, the agent appends a "## Voice reply mode" block to the system prompt each turn. The block tells the model to answer in at most two sentences with a spoken cadence. No markdown, no lists, no code, plain wording for file paths and identifiers. Tool use is unaffected. This is not a separate setting. It follows the text-to-speech checkbox in the settings sidebar, and it starts from the `tts_enabled` value in the `settings.json` voice block. Mechanism: a shared `voice_mode_flag` (`Arc<AtomicBool>`) on `AgentLoop`, read each turn in `sync_dynamic_config` alongside the effort flag and model name, driving `MessageHistory::set_system_suffix`. The instruction text lives in `voice_mode_instructions()` in `src/agent/prompt.rs`. The GUI holds the same flag and writes it on every text-to-speech toggle. This is separate from and additional to `filter_for_speech` in `src/voice/mod.rs`, which still strips markdown and caps spoken length on the reply the agent sends back. The prompt shortens the reply at the source. The filter cleans whatever comes back regardless.
...
`ImageAttachment { data, media_type }`, also in `src/api/types.rs`, is the backend-agnostic outgoing type a turn carries before it is mapped onto any one backend's wire shape: `data` is base64 with no `data:` prefix, `media_type` is a MIME string like `"image/png"`. `build_user_content` in `src/agent/agent_loop.rs` maps it per provider: `Content::text(text)` unchanged for a `None` image, the OpenAI `image_url` part built with a `data:` URL for Ollama, and a dropped attachment plus a transcript notice for DeepSeek, since DeepSeek accepts no image part at all and there is no point waiting on the 400 that would only confirm what is already known. `Backend::run_with_image` (`src/backend/mod.rs`) and `ClaudeCliDriver::send_with_image` (`src/backend/claude_cli/process.rs`) carry the attachment down to each backend kind; `build_user_turn_line` in the same file builds the Anthropic `image` content block for the stdin turn. The old entry points, `Backend::run` and `ClaudeCliDriver::send`, delegate to the `_with_image` versions with `None`, so a turn with no image is unaffected either way.
...
`Settings::save` writes pretty JSON to `settings.json` in the given directory. The GUI holds a `Settings` value and the project root, calls one small `apply_*` free function per control, then calls `persist_settings`. Every `apply_*` function is a plain function over `&mut Settings`, so a round-trip test can call it without building a GUI. Each writer lives with the control it serves: the voice writers in `src/gui/voice_ui.rs`, the backend and model writers in `src/gui/backend_picker.rs`, the two Autopilot writers in `src/gui/autopilot_tab.rs`, and the rest in `src/gui/settings_panel.rs`. `src/gui/mod.rs` holds none of them. A save failure is logged at `warn` and otherwise ignored: losing a preference must never take the session down.
...
**Multi-turn subagent sessions:** a `keep_open: true` `Task` dispatch does not end when the call returns. Its backend stays alive, held by `SubagentRegistry` in `src/backend/registry.rs`, one registry per `AgentLoop`, not one shared across the whole dispatch tree. That matters for the lifetime rule below: each agent owns only the sessions it opened, so one agent's turn end can never close a session a different agent still has open.
...
- Image input: `Content`/`ContentPart` in `src/api/types.rs`, `ImageAttachment` and `build_user_content` in `src/agent/agent_loop.rs`, `Backend::run_with_image` and `ClaudeCliDriver::send_with_image` in `src/backend/mod.rs` and `src/backend/claude_cli/process.rs`, the pending-attachment strip, drag-and-drop, and Ctrl+V paste in `src/gui/mod.rs`, `BlockKind::Image` and its rendering in `src/gui/transcript.rs` and `src/gui/mod.rs`, `src/tools/read_image.rs` (`ReadImageTool`), and the image-eliding pass of pruner tier one in `src/agent/pruning.rs`. Per-backend support was checked against real requests, recorded in `docs/notes/image-support.md`. See "Image input" above for the full mechanism and the per-backend table.
...
- Subagent blocks: `RoutedEvent` and `RouteHop` in `src/agent/agent_loop.rs`, `spawn_event_forwarder` in `src/backend/subagent.rs`, `BlockKind::Subagent`, `SubagentState`, and `Transcript::apply_routed_event` in `src/gui/transcript.rs`, and `render_subagent_block` in `src/gui/mod.rs`. See "Subagent blocks" above for the full mechanism.
- Multi-turn subagent sessions: `src/backend/registry.rs` (`SubagentRegistry`, the lifetime rule, and both runaway-cost caps), `src/backend/stub.rs` (`StubBackend`, the scripted backend subagent tests run against), `src/tools/send_message.rs` (`SendMessageTool`), `src/tools/close_session.rs` (`CloseSessionTool`), and the long-lived-driver branch of `run_subagent` in `src/backend/subagent.rs` for a `keep_open` `claude_cli` dispatch. See "Multi-turn subagent sessions" above for the full mechanism.
- Working directory: `src/tools/cd.rs` (`CdTool`), the `working_dir` field on `BackendFactory` and `BackendFactory::with_working_dir`, `MessageHistory::set_working_dir` in `src/agent/history.rs`, the directory-change respawn branch of `ClaudeCliDriver::ensure_ready` in `src/backend/claude_cli/process.rs`, the `working_dir` override on `Task`, and the sidebar field and status bar reading in `src/gui/mod.rs`. See "Working directory" above for the full mechanism.
...
- Session management: `src/session/mod.rs` (`SessionId`, `SessionMeta`, `SessionRecord`, `derive_title`), `src/session/store.rs` (`SessionStore`, the disk layer), `src/gui/session_state.rs` (`SessionState`, the save/switch/load/reset state), `src/gui/sessions_tab.rs` (the Sessions tab). `AgentCommand` in `src/agent/agent_loop.rs` replaced the old bare `String` input channel. `--resume` was verified against the real `claude` binary. See "Session persistence" above for the full mechanism. The Sessions tab UI still needs a human's visual confirmation.
...
**Tests:** 812 lib tests, and 20 integration tests across three targets, all passing. The binary target carries 0 tests. `backend_resolution_tests` moved out of `src/main.rs`. It now lives in `src/backend/factory.rs`, as `factory_tests.rs`, covering `resolve_active_backend`, `may_dispatch`, the depth-gated `Task`, `SendMessage`, and `CloseSession` tool wiring, and `with_working_dir`, confirming an override never moves the parent's `Arc`. No failures, no ignored tests. `voice/stt.rs` and `voice/tts.rs` each carry one more test that needs the Whisper and Kokoro model files on disk, see `docs/voice-setup.md`. Those two sit behind the `voice-models` cargo feature, off by default, and run with `cargo test --features voice-models`, which brings the lib total to 814.

`tests/api_turn.rs` is the crate's first integration test target, with 12 tests. It points a real `ApiClient` at a local `wiremock` server through `BackendConfig`'s existing optional `base_url`, so no production code changed to add it. `wiremock` is a dev dependency. Its tests: a full turn read back from a canned SSE stream, a tool call round trip checked against the recorded second request body, a retry that follows a 500, a malformed chunk that gets skipped without failing the turn, a stream that never sends the `[DONE]` sentinel but still terminates, two tests added for phase 5 that run a real turn per effort level against the mock server, and five tests added for phase 6 that check image handling on the wire against the mock server. One phase 5 test confirms Ollama produces five distinct `reasoning_effort` values on the wire, one per level. The other confirms DeepSeek produces the documented non-uniform `thinking_mode` mapping, pinning the two levels (`Medium` and `High` collapsing onto `thinking`) a boolean toggle could never have reached. Of the phase 6 tests: one confirms Ollama sends a pasted image as an OpenAI `image_url` part, one confirms DeepSeek drops the same image and posts a transcript notice naming the backend instead, one confirms a turn with no image is unaffected on either provider, and two more do the same pair of checks for an image a tool hands back through `ToolOutput::image` rather than a pasted one, confirming the synthetic `Role::User` message path reaches the wire the same way.

`src/bin/fake_claude.rs` is a fake `claude` binary, built for `tests/claude_cli_fake_binary.rs`, with 1 test. It emits a `system` init event carrying a session id, echoes each stdin turn back as a `text_delta`, and ends every turn with its own `result` event. The test finds it through `env!("CARGO_BIN_EXE_fake_claude")` and points a real `ClaudeCliDriver` at it through the existing `CLAUDE_CLI_PATH` override, so no second hook was added and an unset override leaves the spawn path untouched. It also honours `--resume` by echoing the id back, and it carries two markers a test can send: one makes a turn hang, one makes the process exit after replying, since its normal reply is near-instant and an interrupt-mid-turn test and a self-exit test would otherwise be out of reach.

`tests/claude_cli_lifecycle.rs` holds 7 tests covering the process lifecycle that had never been tested: a turn boundary across two turns, an interrupt killing the child with the next turn still working, a respawn after the child exits on its own, a voice-mode flip respawning, a working-directory change respawning, plus two more added by a real production defect these tests found and fixed. `ClaudeCliDriver` used to kill and respawn its child on turn 2 of every ordinary conversation: `drain_session_id` learned the running child's session id but never updated `spawned_resume_id`, so `ensure_ready` compared the newly learned id against a stale `None`, read it as a change, and restarted a healthy child. `drain_session_id` now sets both fields, since the running child is the session whose id it just reported. `turn_two_of_an_ordinary_conversation_does_not_respawn_the_child` was confirmed red against the old code before the fix. Loading a different session still respawns with `--resume`, covered by its own test. These tests assert on the child's process id, not the session id, because a genuine resume deliberately keeps the session id stable across a respawn. The `claude_cli` spawn arguments themselves are still covered by index-based unit tests in `src/backend/claude_cli/process.rs` that need no real binary; what the fake binary and the lifecycle tests add is coverage of the respawn timing and turn-boundary behavior a unit test cannot reach without a real child process.
