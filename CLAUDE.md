...
`Settings::save` writes pretty JSON to `settings.json` in the given directory. The GUI holds a `Settings` value and the project root, calls one small `apply_*` free function per control, then calls `persist_settings`. Every `apply_*` function is a plain function over `&mut Settings`, so a round-trip test can call it without building a GUI. The sidebar's own `apply_*` writers live in `src/gui/settings_panel.rs`, alongside `render_settings_panel` and the rest of the sidebar-only code (see the GUI entry under "Done" below). `src/gui/mod.rs` keeps only the two Autopilot-tab writers, `apply_autopilot_task` and `apply_autopilot_iterations`. A save failure is logged at `warn` and otherwise ignored: losing a preference must never take the session down.
...
| `gui/session_state.rs` | 0 (its behavior is covered by `gui/mod.rs` tests) |
