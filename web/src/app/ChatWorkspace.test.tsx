...
it('submits accepted text and image identities and displays ordered running and terminal projection', async () => {
  ...
  const { rerender } = render(<ChatWorkspace acceptedAttachmentId="image-1" operation={null} send={send} transcript={[]} />);
  await userEvent.type(screen.getByLabelText('Message'), 'hello');
  await userEvent.click(screen.getByRole('button', { name: 'Send message' }));
  expect(send).toHaveBeenCalledWith({ command: 'send_message', payload: { text: 'hello', attachment_id: 'image-1' } });

  rerender(<ChatWorkspace operation={{ kind: 'chat', operation_id: 'chat-1', phase: 'running', progress: null, message: 'Generating response', error: null }} send={send} transcript={[
  ...
  rerender(<ChatWorkspace operation={{ kind: 'chat', operation_id: 'chat-1', phase: 'completed', progress: null, message: 'Response complete', error: null }} send={send} transcript={[{ id: 6, type: 'terminal', outcome: 'completed', message: 'Response complete' }]} />);
