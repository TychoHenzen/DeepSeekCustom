...
/// does not claim every core on a large machine. `pub`: the moved bounds
/// test checks the returned thread count against this constant directly.
pub const MAX_THREADS: i32 = 8;
...
/// alongside it. `pub`: the moved bounds test calls this directly; it is
/// also used unconditionally by `build_params` above.
pub fn sensible_thread_count() -> i32 {
